...
import { ArrowRight, Activity, Database, Shield, Zap, Sparkles } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-blue-500/30">
      ...
      <main className="flex-grow relative overflow-hidden flex flex-col items-center">
        ...
        <section className="w-full relative py-24 md:py-36 px-4 flex flex-col items-center justify-center text-center">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed font-light">
              Transform unstructured PDF result sheets into clean, structured Excel data in seconds. Built for speed, accuracy, and edge-native performance.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <motion.div 
                  ...
                  onHoverStart={() => setPingingCards(prev => ({...prev, [idx]: true}))}
                  className={`bg-slate-950/50 border border-slate-800 p-8 rounded-[2rem] transition-colors group ${pingingCards[idx] ? 'animate-ping' : 'hover:bg-slate-800/50'}`}
                >
        ...
        <section className="w-full py-12 px-4 mb-6">
          <motion.div 
            ...
            className={`max-w-6xl mx-auto rounded-[2rem] p-12 md:p-16 relative overflow-hidden border border-slate-800 shadow-2xl ${statsPinged ? 'animate-ping' : 'hover:bg-slate-800/50'}`}
          >            
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 text-center items-center">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1, duration: 0.5, ease: "easeOut" }}>
                <div className="text-5xl font-black text-white/90 mb-2 tracking-tighter">₹0</div>
                <div className="text-sm font-medium text-slate-500 uppercase tracking-widest">Cost</div>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.5, ease: "easeOut" }}>
                <div className="text-5xl font-black text-white/90 mb-2 tracking-tighter">&lt;5s</div>
                <div className="text-sm font-medium text-slate-500 uppercase tracking-widest">Extraction</div>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.5, ease: "easeOut" }}>
                <div className="text-5xl font-black text-white/90 mb-2 tracking-tighter">100%</div>
                <div className="text-sm font-medium text-slate-500 uppercase tracking-widest">Serverless</div>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.5, ease: "easeOut" }}>
                <div className="flex flex-col items-center justify-center">
                  <Activity className="w-10 h-10 text-white/90 mb-2 drop-shadow-[0_0_15px_rgba(52,211,153,0.5)]" />
                  <div className="text-sm font-medium text-slate-500 uppercase tracking-widest">System Online</div>
                </div>
              </motion.div>
            </div>
          </motion.div>
